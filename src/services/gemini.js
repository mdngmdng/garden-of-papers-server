...
async function summarizePaper(paragraph, marker, paperTitle, sentences) {
  ...
  const prompt = `You are a scientific paper analyst. Summarize the following paper focusing on the perspective described in the citation context.

## Citation context (from another paper, mentioning this paper as ${marker})
${paragraph}
...
## Instructions
위 citation context가 이 논문을 인용한 맥락과 관점을 중심으로 "배경", "기여", "한계"를 한글로 요약해 주세요.
- **배경**: citation context에서 언급된 문제/동기와 연결하여, 이 논문이 다루는 연구 배경을 설명하세요.
- **기여**: citation context에서 이 논문을 인용한 이유(어떤 방법/결과가 활용되었는지)를 중심으로 기여를 구체적으로 설명하세요.
- **한계**: citation context의 관점에서 이 논문이 다루지 못한 부분이나 후속 연구의 필요성을 설명하세요.
 ...
 */
async function storytelling(papers, links, myResearch = '', clientPrompt = '') {
  ...
  let prompt;
  if (clientPrompt) {
    // 클라이언트에서 보낸 프롬프트의 플레이스홀더를 치환
    prompt = clientPrompt
      .replace('{myResearch}', myResearchSection)
      .replace('{paperList}', paperList)
      .replace('{linkList}', linkList);
  } else {
    prompt = `You are an academic writing assistant. Write a "Related Work" section in Korean based on the following papers and their citation relationships.
${myResearchSection}
...
2. "본 연구", "본 논문", "우리" 같은 1인칭·자기 연구 표현은 오직 마지막 문단에서 저자의 연구를 소개할 때만 한 문장만 사용하세요.
